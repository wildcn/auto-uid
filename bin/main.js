...
program.version(packJSON.version).option("-a, --auto", "使用 -s 初始化项目配置，并执行 -f 全量匹配并添加唯一ID").option("-s, --setup", "初始化项目配置，在根目录下生成feuid2.config.js、.huskyrc.json").option("-f, --full", "处理所有匹配的文件").option("-t, --target <target>", "处理指定文件").option("-r, --dir <dir>", "处理指定目录下的文件,多目录以,分隔").option("-u, --update", "更新已经生成的唯一ID").option("-p, --path <path>", "自定义项目路径，默认为当前路径");
